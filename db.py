...
# ── Migration: import existing trades.json on first deploy ─

def migrate_from_json(data_dir: str):
    """
    One-time migration: imports trades.json into the trades table.
    Safe to call on every startup — skips if trades table already has rows.
    """
    conn = get_conn()
    cur = conn.cursor()
    ...
    if cur.fetchone()[0] > 0:
        cur.close()
        conn.close()
        print("[db] Migration skipped — trades table already populated", flush=True)
        return
    ...
    if not _os.path.exists(trades_path):
        cur.close()
        conn.close()
        print("[db] Migration skipped — no trades.json found", flush=True)
        return

    try:
        with open(trades_path, "r") as f:
            data = json.load(f)
        trades = data.get("paper", [])
        for t in trades:
            cur.execute("""
                INSERT INTO trades (
                    time, date, instrument, direction, timeframe,
            ...
            """, {
                ...
                "pnl":            t.get("pnl", 0),
                "oanda_trade_id": t.get("oanda_trade_id"),
            })
        conn.commit()
        print(f"[db] Migrated {len(trades)} trades from trades.json", flush=True)
    except Exception as e:
