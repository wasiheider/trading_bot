...
def place_limit_order(instrument: str, direction: str, lot_size: float, price: float, sl_price: float = None) -> dict:
    """Place a GFD limit order at price. Returns dict with trade_id (None if pending) and pending flag.
    Raises RuntimeError if OANDA rejects the limit price (price already past the level)."""
    oanda_instrument = INSTRUMENT_MAP.get(instrument.upper())
    if not oanda_instrument:
        raise ValueError(f"No OANDA mapping for: {instrument}")
    ...
    if "orderFillTransaction" in resp:
        ...
        closed_ids = [c.get("tradeID") for c in fill.get("tradesClosed", [])]
        raise RuntimeError(f"Limit order filled but no new trade opened — closed existing: {closed_ids}")
    ...
    if "orderCancelTransaction" in resp:
        reason = resp["orderCancelTransaction"].get("reason", "unknown")
        raise RuntimeError(f"Limit order cancelled: {reason}")
